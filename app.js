...
function applyFieldErrors(errorsByType) {
  document.querySelectorAll('#forms input[data-type][data-key]').forEach((input) => {
    const type = input.dataset.type;
    const key = input.dataset.key;
    if (errorsByType[type]?.includes(key)) {
...
function renderOutput() {
  ...
  applyFieldErrors(errorsByType);
}

function syncOutputOnly() {
  saveState();
  renderOutput();
}

function syncAndRenderFull() {
  saveState();
  renderPresets();
  renderTypes();
  renderForms();
